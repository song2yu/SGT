# Adapted from the VIEScore project by TIGER-AI-Lab:
#   https://github.com/TIGER-AI-Lab/VIEScore
# See the upstream repository for the original license.
...
import json
os.environ['OPENAI_API_KEY'] = "9oZlXZSeNunLwfg0LgKOzyJT@3695"
...
class GPT4v():
    def __init__(self, api_key_path='keys/secret.env', are_images_encoded=False, model_name="gpt-4-vision-preview", 
                 base_url=None):  # optional ``base_url`` for a custom OpenAI-compatible endpoint
        ...
        Args:
            ...
            base_url (str): API base URL for company internal proxy.
        """
        self.multiple_api_keys = False
        self.current_key_file = None
        self.key_lists = None
        self.base_url = base_url  # persist the base_url
        ...
        if isinstance(api_key_path, list):
            ...
            # Decide whether the input is a file path or a raw API key.
        ...
        # Use the configured base_url when building the client.
        self.client = OpenAI(
            api_key=self.api_key,
            base_url=self.base_url  # forward the base_url
        )
...
class GPT4o(GPT4v):
    def __init__(self, api_key_path='keys/secret.env', are_images_encoded=False, model_name="gpt-4o",
                 base_url=None):  # optional ``base_url`` for a custom OpenAI-compatible endpoint
        super().__init__(api_key_path, are_images_encoded, model_name, base_url)


if __name__ == "__main__":
    # Usage example.
    model = GPT4o(
        api_key_path='YOUR_OPENAI_API_KEY_OR_FILE_PATH',  # pass a raw token or a path to a file containing one
        model_name="gpt-4o",
        base_url=None  # use a custom OpenAI-compatible endpoint, or None to use the default
    )
