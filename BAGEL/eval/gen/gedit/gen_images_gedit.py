...
def setup_models(model_path, origin_model_path, device=0, use_ema=False):
    """
    ...
    # Load LLM
    llm_config = Qwen2Config.from_pretrained(os.path.join(origin_model_path, "llm_config.json"))
    llm_config.qk_norm = True
    llm_config.tie_word_embeddings = False
    llm_config.layer_module = "Qwen2MoTDecoderLayer"
    
    # Load Vision model
    vit_config = SiglipVisionConfig.from_json_file(os.path.join(origin_model_path, "vit_config.json"))
    vit_config.rope = False
    vit_config.num_hidden_layers = vit_config.num_hidden_layers - 1
    
    # Load VAE
    vae_model, vae_config = load_ae(local_path=os.path.join(origin_model_path, "ae.safetensors"))
    ...
    # Tokenizer Preparing
    tokenizer = Qwen2Tokenizer.from_pretrained(origin_model_path)
    tokenizer, new_token_ids, _ = add_special_tokens(tokenizer)

    if use_ema:
        state_dict_path = os.path.join(model_path, f"ema.safetensors") # may beed to change
        state_dict = load_file(state_dict_path, device="cpu")
    else:
        state_dict_path = os.path.join(model_path, f"model.safetensors")
        state_dict = load_file(state_dict_path, device="cpu")
    ...
    if use_think: 
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            ...
            unpacked_latent = model.generate_text(
                ...
                max_length=10240,
                do_sample=True if temperature > 0 else False,  # sample only when temperature > 0
                temperature=temperature if temperature > 0 else 1.0,  # disable sampling when temperature == 0
...
def process_dataset(
    ...
    cfg_type="serial_text_img", num_samples=None, shard_id=0, total_shards=1, 
    use_think=False, device='cuda', seed=42, temperature=0.3  # seed and temperature args
):
    """
        ...
        try:
            with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
                edited_image = editing_with_text_img_cfg(
                    ...
                    timestep_shift=3.0,
                    seed=seed,  # forward the seed
                    temperature=temperature,  # forward the temperature
...
def main():
    ...
    parser.add_argument('--use-ema', action='store_true', 
                        help='Whether to use the EMA weights for generation.')
    
    # Added args for random seed and temperature.
    parser.add_argument("--seed", type=int, default=42, 
                        help="Random seed for reproducibility")
    parser.add_argument("--temperature", type=float, default=0.3, 
                        help="Temperature for text generation in think mode (0 for deterministic)")

    args = parser.parse_args()
    
    # Set the global random seed at startup.
    set_seeds(args.seed)
    
    # Setup models
    model, vae_model, tokenizer, new_token_ids, vae_transform, vit_transform = setup_models(
        args.model_path, args.origin_model_path, args.device, args.use_ema
    )
    ...
    process_dataset(
        ...
        device=args.device,
        seed=args.seed,  # forward the seed
        temperature=args.temperature,  # forward the temperature
    )
