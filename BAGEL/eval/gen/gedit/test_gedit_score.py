...
# Make sure the viescore directory is on PYTHONPATH before running this file,
# e.g. `export PYTHONPATH="./eval/gen/gedit/viescore:$PYTHONPATH"`
    ...
    for retry in range(max_retries):
        try:
            ...
            if retry < max_retries - 1:
                wait_time = (retry + 1) * 2  # back-off: 2s, 4s, 6s, ...
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backbone", type=str, default="gpt4o", choices=["gpt4o", "qwen25vl", "gpt-5"])
    ...
    vie_scores = [VIEScore(backbone=backbone, task="tie", key_path=k) for k in args.gpt_keys]
                ...
                for i, item in enumerate(group_dataset_list):
                    ...
                    if not megfile.smart_exists(save_path_fullset_result_image) or not megfile.smart_exists(save_path_fullset_source_image):
                        if not megfile.smart_exists(save_path_fullset_result_image):
                            print(f"Missing edited image for {sample_prefix}: {save_path_fullset_result_image}")
                        if not megfile.smart_exists(save_path_fullset_source_image):
                            print(f"Missing source image for {sample_prefix}: {save_path_fullset_source_image}")
