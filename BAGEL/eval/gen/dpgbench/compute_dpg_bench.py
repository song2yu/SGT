...
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default='eval/gen/dpgbench/dpg_bench.csv',
    )
    parser.add_argument(
        "--res-path",
        type=str,
...
def compute_dpg_one_sample(args, question_dict, image_path, vqa_model, resolution):
    ...
    key = osp.basename(image_path).split('_')[0]
    ...
    for _ in range(4):
        ...
        for id, question in qid2question.items():
            ...
            with open(args.res_path.replace('.txt', '_detail.txt'), 'a') as f:
                f.write(image_path + ', ' + str(_) + ', ' + question + ', ' + answer + '\n')
...
def main():
    ...
    try:
        ...
        # Only the main process downloads / initialises the model; the others wait.
        if accelerator.is_main_process:
            print("Main process is downloading / initialising the model ...")
            if args.vqa_model == 'mplug':
                vqa_model = MPLUG(device=device)
            else:
                raise NotImplementedError(f"VQA model {args.vqa_model} is not implemented.")
        
        # Wait for the main process to finish downloading the model.
        accelerator.wait_for_everyone()
        
        # Other processes can now safely initialise the model.
        if not accelerator.is_main_process:
            print(f"Process {accelerator.process_index} is initialising the model ...")
            if args.vqa_model == 'mplug':
                vqa_model = MPLUG(device=device)
            else:
                raise NotImplementedError(f"VQA model {args.vqa_model} is not implemented.")
        ...
        print(f'Process {local_rank} is starting evaluation of {model_id}, handling {len(local_filename_list)} files.')
        
        for fn in tqdm(local_filename_list, desc=f"process {local_rank}"):
            ...
            try:
                ...
                print(f'Process {local_rank} failed to handle file: {fn}, error: {e}')
                continue
        ...
        print(f"Process {accelerator.process_index} hit an error: {e}")
        raise
    
    finally:
        # Make sure to clean up the distributed process group properly.
        print(f"Process {accelerator.process_index} is cleaning up ...")
        try:
            ...
            # If torch.distributed was initialised, tear down the process group.
            if torch.distributed.is_initialized():
                torch.distributed.destroy_process_group()
                
        except Exception as cleanup_error:
            print(f"Warning during process cleanup: {cleanup_error}")
