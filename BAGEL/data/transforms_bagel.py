...
def motion_blur_opencv(image, kernel_size=15, angle=0):
    ...
    if img.ndim == 2:
        ...
        # Convolve each channel of a color image independently.
...
def shuffle_patch(image, num_splits, gap_size=2):
    """Split an image into patches (sizes may differ), shuffle them and
    re-tile with a gap between neighbouring patches."""
    ...
    patch_idx = 0  # Index of the patch currently being placed.
    for i in range(h_splits):
        current_x = 0  # Starting X coordinate of the current column.
        patch_h = patch_heights[i]  # Height of patches in this row.
        for j in range(w_splits):
            # Pick the next shuffled patch.
            patch = patches[patch_idx]
            patch_w = patch_widths[j]  # Width of patches in this column.
            # Paste the patch (top-left corner at (current_x, current_y)).
            new_image.paste(patch, (current_x, current_y))
            # Advance X: next patch position = current width + gap.
            current_x += patch_w + gap_size
            patch_idx += 1
        # Advance Y: next row position = current height + gap.
...
def inpainting(image, num_splits, blank_ratio=0.3, blank_color=(255, 255, 255)):
    """Split an image into patches and randomly blank out some of them
    (useful for simulating simple inpainting-style corruption).

    Args:
        image: input PIL.Image (RGB).
        num_splits: tuple (h_splits, w_splits), number of vertical and
            horizontal splits.
        blank_ratio: float in [0, 1] -- fraction of patches to blank out.
        blank_color: RGB tuple for the blanked area, e.g. (255, 255, 255).

    Returns:
        PIL.Image: the re-tiled image.
    """
    h_splits, w_splits = num_splits
    img_w, img_h = image.size
    ...
    # Allocate the output image with the same size as the input.
    ...
    for i in range(h_splits):
        ...
        patch_h = patch_heights[i]
        for j in range(w_splits):
            # Pick the processed patch.
            patch = processed_patches[patch_idx]
            patch_w = patch_widths[j]
            # Paste it back in its original slot.
