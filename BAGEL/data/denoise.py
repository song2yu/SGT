...
from .add_noise import apply_stochastic_degradation

Image.MAX_IMAGE_PIXELS = 2_000_000_000

class DenoiseDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_denoise_prompt_list
        import random
        self.prompt_templates = get_denoise_prompt_list()
    ...
    def get_training_pair(self, image_file):
        ...
        noisy_numpy_bgr = apply_stochastic_degradation(clean_numpy_bgr)
        
        # 5. Convert the result back (BGR -> RGB) and wrap in a PIL image.
        #    Use cv2 to convert BGR -> RGB.
        noisy_numpy_rgb = cv2.cvtColor(noisy_numpy_bgr, cv2.COLOR_BGR2RGB)
        #    Wrap the NumPy array back into a PIL image.
        noisy_pil_image = Image.fromarray(noisy_numpy_rgb)
        ...
        noisy_tensor = self.vit_transform(noisy_pil_image)
