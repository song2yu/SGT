...
from .add_noise import apply_stochastic_degradation, apply_inpainting_corruption, apply_motion_blur, apply_low_light_corruption, apply_rain_fog_corruption

Image.MAX_IMAGE_PIXELS = 2_000_000_000

class DerainDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_derain_prompt_list
        import random
        self.prompt_templates = get_derain_prompt_list()
    ...
    def get_training_pair(self, image_file):
        ...
        rain_numpy_bgr = apply_rain_fog_corruption(clean_numpy_bgr)
        
        # 5. Convert the result back (BGR -> RGB) and wrap in a PIL image.
        #    Use cv2 to convert BGR -> RGB.
        rain_numpy_rgb = cv2.cvtColor(rain_numpy_bgr, cv2.COLOR_BGR2RGB)
        #    Wrap the NumPy array back into a PIL image.
        rain_pil_image = Image.fromarray(rain_numpy_rgb)
        ...
        rain_tensor = self.vit_transform(rain_pil_image) # vit inputs
