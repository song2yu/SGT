...
class SegmentDataset(DistributedIterableDataset):
    def __init__(
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for item_idx, data_item in enumerate(data_items_, start=item_start_id):
                try:
                    image_file = self.data_dir + data_item['image']
                    edited_file = image_file.replace('sam_selection', 'sam_mask').replace('jpg', 'png') 
                    try:
