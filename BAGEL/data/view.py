...
class ViewDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_view_prompt_list
        import random
        self.prompt_templates = get_view_prompt_list()
    ...
    def __iter__(self):
        ...
        while True:
            ...
            for item_idx, data_item in enumerate(data_items_, start=item_start_id):
                try:
                    image_file = data_item# self.data_dir + data_item
                    edited_file = image_file.replace('left', 'right')
                    try:
