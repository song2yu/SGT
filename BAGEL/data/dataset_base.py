...
class PackedDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        # Keep the list of dataset names for statistics / debugging.
        self.dataset_names = list(data_config.grouped_datasets.keys())
    ...
    def _print_batch_statistics(self, sequence_status, dataset_sample_counts):
        """Print per-batch dataset sample statistics."""
        total_samples = sum(dataset_sample_counts.values())
        sequence_length = sum(sequence_status['sample_lens'])
        
        print(f"\n{'='*60}")
        print(f"Batch Statistics (Rank {self.local_rank})")
        print(f"{'='*60}")
        print(f"Total samples: {total_samples}")
        print(f"Sequence length: {sequence_length}")
        print(f"Dataset sample counts:")
        for name, count in dataset_sample_counts.items():
    ...
    def __iter__(self):
        ...
        # Initialize per-dataset sample counter.
        ...
        while True:
            ...
            if sequence_status['curr'] == 0:
                # Reset counters at the start of a new packed sequence.
                dataset_sample_counts = {name: 0 for name in self.dataset_names}
                    ...
                    if self.is_mandatory[group_index]:
                        while True:
                            ...
                            if num_tokens < self.max_num_tokens_per_sample:
                                ...
                                # Count the sample.
                                dataset_sample_counts[self.dataset_names[group_index]] += 1
                                break
                            else:
            ...
            if sequence_status['curr'] < self.prefer_buffer_before and len(buffer) > 0:
                sample, group_index = buffer.pop(0)
            ...
            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    buffer.append((sample, group_index))
                else:
                    # Optionally print batch statistics for debugging.
                    # self._print_batch_statistics(sequence_status, dataset_sample_counts)
