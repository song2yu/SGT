...
from .add_noise import apply_stochastic_degradation, apply_inpainting_corruption

Image.MAX_IMAGE_PIXELS = 2_000_000_000

class InpaintingDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_inpainting_prompt_list
        import random
        self.prompt_templates = get_inpainting_prompt_list()
    ...
    def get_training_pair(self, image_file):
        ...
        corrupted_numpy_bgr = apply_inpainting_corruption(clean_numpy_bgr)
        
        # 5. Convert the result back (BGR -> RGB) and wrap in a PIL image.
        #    Use cv2 to convert BGR -> RGB.
        corrupted_numpy_rgb = cv2.cvtColor(corrupted_numpy_bgr, cv2.COLOR_BGR2RGB)
        #    Wrap the NumPy array back into a PIL image.
        corrupted_pil_image = Image.fromarray(corrupted_numpy_rgb)
        ...
        corrupted_tensor = self.vit_transform(corrupted_pil_image) # vit inputs
