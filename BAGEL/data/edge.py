...
class EdgeDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_edge_prompt_list
        import random
        self.prompt_templates = get_edge_prompt_list()
        ...
        while True:
            ...
            for item_idx, data_item in enumerate(data_items_, start=item_start_id):
                try:
                    ...
                    edited_file = image_file.replace('train2017', 'coco_edge').replace('jpg', 'png') 
                    try:
