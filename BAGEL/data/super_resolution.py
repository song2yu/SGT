...
from .add_noise import apply_stochastic_degradation, create_sr_pair

Image.MAX_IMAGE_PIXELS = 2_000_000_000

class SuperResolutionDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_super_resolution_prompt_list
        import random
        self.prompt_templates = get_super_resolution_prompt_list()
    ...
    def get_training_pair(self, image_file):
        ...
        high_numpy_rgb = np.array(clean_pil_image)
        #    Use cv2 to convert RGB -> BGR.
        high_numpy_bgr = cv2.cvtColor(high_numpy_rgb, cv2.COLOR_RGB2BGR)

        # 4. Run the degradation pipeline (expects BGR, uint8).
        _, upsampled_lr_numpy_bgr, _ = create_sr_pair(high_numpy_bgr)
        
        # 5. Convert the result back (BGR -> RGB) and wrap in a PIL image.
        #    Use cv2 to convert BGR -> RGB.
        lr_numpy_rgb = cv2.cvtColor(upsampled_lr_numpy_bgr, cv2.COLOR_BGR2RGB)
        #    Wrap the NumPy array back into a PIL image.
        lr_pil_image = Image.fromarray(lr_numpy_rgb)
        ...
        hr_tensor = self.transform(clean_pil_image)  # reconstruction target
        lr_tensor = self.vit_transform(lr_pil_image)  # reconstruction input
