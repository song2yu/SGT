...
from .add_noise import apply_stochastic_degradation, apply_inpainting_corruption, apply_motion_blur, apply_low_light_corruption

Image.MAX_IMAGE_PIXELS = 2_000_000_000

class EnhanceDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_enhance_prompt_list
        import random
        self.prompt_templates = get_enhance_prompt_list()
    ...
    def get_data_paths(self, num_used_data):
        self.all_items = self.read_txt_to_list(self.jsonl_path[0])
    ...
    def get_training_pair(self, image_file):
        
        # 1. Load the image.
        image = Image.open(image_file)
        
        # 2. Normalize into an RGB PIL image (drops any alpha channel).
        clean_pil_image = pil_img2rgb(image)
        
        # --- begin degradation step ---
        
        # 3. PIL (RGB) -> OpenCV/NumPy (BGR).
        #    np.array(pil_image) returns RGB data.
        clean_numpy_rgb = np.array(clean_pil_image)
        #    Use cv2 to convert RGB -> BGR.
        clean_numpy_bgr = cv2.cvtColor(clean_numpy_rgb, cv2.COLOR_RGB2BGR)

        # 4. Run the degradation pipeline (expects BGR, uint8).
        low_light_numpy_bgr = apply_low_light_corruption(clean_numpy_bgr)
        
        # 5. Convert the result back (BGR -> RGB) and wrap in a PIL image.
        #    Use cv2 to convert BGR -> RGB.
        low_light_numpy_rgb = cv2.cvtColor(low_light_numpy_bgr, cv2.COLOR_BGR2RGB)
        #    Wrap the NumPy array back into a PIL image.
        low_light_pil_image = Image.fromarray(low_light_numpy_rgb)
        
        # --- end degradation step ---

        # 6. Apply the transform to both the clean and the degraded image.
        #    self.transform converts a PIL image into a Tensor.
        clean_tensor = self.transform(clean_pil_image)
        low_light_tensor = self.vit_transform(low_light_pil_image) # vit inputs

        # 7. Return the (input, label) pair.
        return low_light_tensor, clean_tensor
        ...
        while True:
            ...
            for item_idx, data_item in enumerate(data_items_, start=item_start_id):
                try:
                    ...
                    try:
                        vit_image_tensor, image_tensor = self.get_training_pair(image_file) # image_tensor: clean_image
                    except Exception as e:
                    ...
                    sample = dict(
                        image_tensor_list=[vit_image_tensor, image_tensor],  # vit_image_tensor: ViT input image; image_tensor: the image to be edited
