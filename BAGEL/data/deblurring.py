...
from .add_noise import apply_stochastic_degradation, apply_inpainting_corruption, apply_motion_blur

Image.MAX_IMAGE_PIXELS = 2_000_000_000

class DeblurringDataset(DistributedIterableDataset):
    def __init__(
        ...
        from data.consts import get_deblurring_prompt_list
        import random
        self.prompt_templates = get_deblurring_prompt_list()
    ...
    def get_training_pair(self, image_file):
        ...
        blurred_numpy_bgr = apply_motion_blur(clean_numpy_bgr)
        
        # 5. Convert the result back (BGR -> RGB) and wrap in a PIL image.
        #    Use cv2 to convert BGR -> RGB.
        blurred_numpy_rgb = cv2.cvtColor(blurred_numpy_bgr, cv2.COLOR_BGR2RGB)
        #    Wrap the NumPy array back into a PIL image.
        blurred_pil_image = Image.fromarray(blurred_numpy_rgb)
        ...
        blurred_tensor = self.vit_transform(blurred_pil_image) # vit inputs
