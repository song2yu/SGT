...
class STEPSftJSONLIterableDataset(DistributedIterableDataset):
    def __init__(
    ...
    def change_format(self, data, num_images):
        ...
        for conversation in data['conversations']:
            if conversation['role'] == 'user':              # 'from'->'role', 'human'->'user'
                if '<image>' not in conversation['content']: # 'value'->'content'
                    elements.append({
                        'type': 'text',
                        'has_loss': 0,
                        'text': conversation['content'],     # 'value'->'content'
                    })
                else:
                    text_list = conversation['content'].split('<image>')  # 'value'->'content'
            ...
            elif conversation['role'] == 'assistant':       # 'from'->'role', 'gpt'->'assistant'
                elements.append({
                    'type': 'text',
                    'has_loss': 1,
                    'text': conversation['content'],         # 'value'->'content'
