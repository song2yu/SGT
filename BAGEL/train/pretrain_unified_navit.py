...
import pdb
import wandb
wandb.init(project="bagel", mode="offline")
...
class ModelArguments:
    model_path: str = field(
        ...
        metadata={"help": "Path to the directory containing the pretrained model files."}
    )
    llm_path: str = field(
    ...
    max_latent_size: int = field(
        default=64,
        metadata={"help": "Maximum latent grid size (patchmodel_pathes per side) for the VAE latent tensor."}
    )
...
class DataArguments:
    ...
    prefetch_factor: int = field(
        default=2, # debug: set to None if num_workers == 0  else 2
        metadata={"help": "How many batches each DataLoader worker pre-loads in advance."}
    )
    num_workers: int = field(
        default=1, # debug for 0 else 1
...
class TrainingArguments:
    ...
    save_after: int = field(
        default=3500,# 4k 
        metadata={"help": "Save the checkpoint after N training steps."}
    )
    ...
    expected_num_tokens: int = field(
        default=8192*2, # 32768-8   4gpus-4096*2   32768  8192
        metadata={"help": "Soft target token count; yield the batch once it reaches or exceeds this size."}
    )

    # --- distributed training / FSDP ---
    # Number of model replicas (parallel copies across GPUs).
    num_replicate: int = field(
        default=1, # 2
        metadata={"help": "Number of model replicas per GPU rank for tensor parallelism."}
    )
    # How many groups to split the GPUs into for sharding.
    num_shard: int = field(
        default=8, # 4 debug 1
        metadata={"help": "Number of parameter shards when using FSDP HYBRID_SHARD."}
    )
    # Core FSDP sharding strategy -- controls how parameters, gradients and
    # optimizer state are sharded across GPUs:
    #   FULL_SHARD    -- fully shard everything (params + grads + opt state).
    #                    Lowest memory, highest communication cost.
    #   HYBRID_SHARD  -- combine FSDP with intra-node tensor parallelism:
    #                    FULL_SHARD inside a node, then cross-node comm.
    #   SHARD_GRAD_OP -- only shard grads / opt state; keep full param
    #                    replicas on every GPU. Uses more memory but less comm.
    sharding_strategy: str = field(
        default="HYBRID_SHARD",
        metadata={"help": "FSDP sharding strategy: FULL_SHARD, SHARD_GRAD_OP, HYBRID_SHARD, etc."}
    )
    # Pre-fetch the next layer's parameters while the backward pass runs.
    backward_prefetch: str = field(
        default="BACKWARD_PRE",
        metadata={"help": "FSDP backward prefetch strategy (BACKWARD_PRE or NO_PREFETCH)."}
    )
    # Memory optimization: offload unused tensors from GPU to CPU memory.
    ...
    freeze_und: bool = field(
        default=True,
        metadata={"help": "Freeze the visual understanding transformers."}
    )    
    ...
    use_flex: bool = field(
        default=True, # flex_attn default set to True
...
def make_freeze_and_print_hook(name, do_print=True):
    def hook(grad):
        if do_print:
            print(f"[before freeze] {name}: norm={grad.norm().item():.6e}")
        zero_grad = torch.zeros_like(grad)
        if do_print:
            print(f"[after  freeze] {name}: norm={zero_grad.norm().item():.6e}")  # always 0
        return zero_grad
    return hook
...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        # wandb mode priority:
        #   1) the WANDB_MODE environment variable (disabled / offline / online);
        #   2) the --wandb_offline True command-line flag -> offline;
        #   3) default to online.
        _env_wandb_mode = os.environ.get("WANDB_MODE", "").strip().lower()
        if _env_wandb_mode in ("disabled", "offline", "online"):
            _wandb_mode = _env_wandb_mode
        elif training_args.wandb_offline:
            _wandb_mode = "offline"
        else:
            _wandb_mode = "online"

        if _wandb_mode == "disabled":
            # Fully disable wandb: no login, no network, no disk writes;
            # wandb.log() becomes a no-op.
    ...
    if training_args.freeze_llm:  # freeze the LLM during training
    ...
    if training_args.freeze_vit and training_args.visual_und:
        ...
        for param in model.vit_model.parameters():
            param.requires_grad = False
    if training_args.freeze_und and training_args.visual_und: # training_args.freeze_connector:#
        for param in model.connector.parameters():
            param.requires_grad = False

    #### for single gpu debug        
    # del model.language_model.model.layers[2:26]
    ...
    ########################  hook here #########################
    # freeze_llm_w_hooks = False
    # if freeze_llm_w_hooks:
    #     freeze_hooks = []
    #     for name, param in fsdp_model.named_parameters():
    #         if "language_model" in name and param.requires_grad:
    #             h = param.register_hook(make_freeze_and_print_hook(name, do_print=False))
    #             freeze_hooks.append(h)

    ########################  training here #########################
    for curr_step, data in enumerate(train_loader, start=train_step):
        # data['packed_vit_token_indexes'].shape: 6656
        # data['mse_loss_indexes'].shape: 26624
        # data['packed_vae_token_indexes'].shape: 26624
        # data['mse_loss_indexes']==data['packed_vae_token_indexes'] is true
        # sequence_length: 33961
        data = data.cuda(device).to_dict()
        data_indexes = data.pop('batch_data_indexes', None)
        ce_loss_weights = data.pop('ce_loss_weights', None)

        gen_flag = data.get('padded_images') is not None
        with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
            if training_args.visual_gen and gen_flag:
                with torch.no_grad():
        ...
        if curr_step % training_args.log_every == 0:
            ...
            # Measure the time taken by the all_reduce call.
            start_allreduce_time = time()
            dist.all_reduce(total_samples, op=dist.ReduceOp.SUM)
            allreduce_time = time() - start_allreduce_time
            ...
            for key, value in loss_dict.items():
                ...
                start_time_reduce = time()
                dist.all_reduce(avg_loss, op=dist.ReduceOp.SUM)
                reduce_time = time() - start_time_reduce
                avg_loss = avg_loss.item() / dist.get_world_size()
                message += f"Train Loss {key}: {avg_loss:.4f}, "
                wandb_log[key] = avg_loss
            message += f"Train Steps/Sec: {steps_per_sec:.2f}, "
            if curr_step % (training_args.log_every * 10) == 0 and dist.get_rank() == 0:
                message += f"AllReduce Time: {allreduce_time*1000:.2f}ms, "
            logger.info(message)
            ...
            wandb_log['allreduce_time_ms'] = allreduce_time * 1000
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0 and curr_step >= training_args.save_after:
            dist.barrier()
            ...
            try:
                ...
                safe_data_status = dict(data_status) if data_status is not None else {}
                
                dist.gather_object(safe_data_status, gather_list, dst=0)
