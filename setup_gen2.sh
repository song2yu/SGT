#!/usr/bin/env bash
# setup_gen2.sh - Create the OmniGen2 environment on a local disk via uv and install deps.
# Design principles:
...
PROJECT_DIR="OmniGen2"
FLASH_ATTN_VERSION="2.7.4.post1"

# venv location: prefer local-ssd, fall back to /root (overlay).
# Override via the VENV_ROOT env var, e.g.: VENV_ROOT=/local-ssd/venvs ./setup_gen2.sh
if [[ -n "${VENV_ROOT:-}" ]]; then
    VENV_BASE="${VENV_ROOT}"
...
echo ""
echo "========================================"
echo "[DONE] OmniGen2 environment is ready!"
echo ""
