...
if is_triton_available(): #  and False
...
class OmniGen2Transformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def img_patch_embed_and_refine(
        ...
        # pdb.set_trace()
        batch_size = len(hidden_states) # 1 x 1040 x 64
    ...
    def forward(
        self,
        hidden_states: Union[torch.Tensor, List[torch.Tensor]], # 16x52x80 
        timestep: torch.Tensor,
        text_hidden_states: torch.Tensor,
        freqs_cis: torch.Tensor,
        text_attention_mask: torch.Tensor,
        ref_image_hidden_states: Optional[List[List[torch.Tensor]]] = None, # 16x52x80 input_latent
