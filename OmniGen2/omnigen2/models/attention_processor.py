...
class OmniGen2AttnProcessorFlash2Varlen:
    ...
    def __call__(
        ...
        # print(f"Attention forward time: {end_time - start_time:.4f} seconds")
...
class OmniGen2AttnProcessor:
    ...
    def __call__(
        ...
        # import time
        # torch.cuda.synchronize()
        # time_306 = time.perf_counter()
        # Get Query-Key-Value Pair
        query = attn.to_q(hidden_states)
        key = attn.to_k(encoder_hidden_states)
        value = attn.to_v(encoder_hidden_states)
        # torch.cuda.synchronize()
        # time_312 = time.perf_counter()
        # print(f'306-312: {time_312-time_306}')
        query_dim = query.shape[-1]
        inner_dim = key.shape[-1]
        head_dim = query_dim // attn.heads
        dtype = query.dtype
        ...
        query = query.view(batch_size, -1, attn.heads, head_dim)
        key = key.view(batch_size, -1, kv_heads, head_dim)
        value = value.view(batch_size, -1, kv_heads, head_dim) # b s n d
        # torch.cuda.synchronize()
        # time_325 = time.perf_counter()
        # print(f'312-325: {time_325-time_312}')
        # Apply Query-Key normalization
        if attn.norm_q is not None:
            attn.norm_q.weight.to(query.dtype)
            query = attn.norm_q(query)
        if attn.norm_k is not None:
            attn.norm_k.weight.to(key.dtype)
            key = attn.norm_k(key)
        # torch.cuda.synchronize()
        # time_335 = time.perf_counter()
        # print(f'325-335: {time_335-time_325}')
        # Apply Rotary Position Embeddings
        if image_rotary_emb is not None:
            query = apply_rotary_emb(query, image_rotary_emb, use_real=False)
            key = apply_rotary_emb(key, image_rotary_emb, use_real=False)
        ...
        query, key = query.to(dtype), key.to(dtype)
        # torch.cuda.synchronize()
        # time_344 = time.perf_counter()
        # print(f'335-344: {time_344-time_335}')
        # Calculate attention scale
        if base_sequence_length is not None:
            softmax_scale = math.sqrt(math.log(sequence_length, base_sequence_length)) * attn.scale
        else:
            softmax_scale = attn.scale
        # softmax_scale = torch.tensor(softmax_scale, dtype=query.dtype, device=query.device)
        ...
        query = query.transpose(1, 2) # d=120  n=21
        key = key.transpose(1, 2)
        value = value.transpose(1, 2)
        # torch.cuda.synchronize()
        # time_362 = time.perf_counter()
        # print(f'344-362: {time_362-time_344}')
        ...
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask.to(value.dtype), scale=softmax_scale) # .to(value.dtype)
        # torch.cuda.synchronize()
        # time_375 = time.perf_counter()
        # print(f'362-375: {time_375-time_362}')
