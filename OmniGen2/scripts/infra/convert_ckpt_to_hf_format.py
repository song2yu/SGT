...
def main(args):
    ...
    state_dict = torch.load(model_path, mmap=False, weights_only=False, map_location=torch.device('cpu')) # , map_location=torch.device('cpu')
    missing, unexpect = transformer.load_state_dict(
        state_dict['module'], assign=True, strict=False
    )
...
def parse_args():
    ...
    parser.add_argument("--config_path", type=str, default='experiments/ft_lora_bs_2_ori/ft_lora.yml')
    parser.add_argument("--model_path", type=str, default='/experiments/ft_lora_bs_2_ori/checkpoint-4000/pytorch_model/mp_rank_00_model_states.pt')
    parser.add_argument("--save_path", type=str, default='experiments/ft_lora_bs_2_ori/transformer_lora')
