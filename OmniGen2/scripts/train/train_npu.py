...
# Optional: log in to HuggingFace Hub via the HF_TOKEN environment variable.
...
def parse_args(root_path) -> OmegaConf:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default='options/ft_lora_debug.yml',
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        # default='data_configs/train/example/edit/edit.yml',
        help="Data path.",
    )
...
def main(args):
    ...
    model = OmniGen2Transformer2DModel.from_pretrained(
        args.model.pretrained_model_path, subfolder="transformer", attn_implementation='eager'# ,device_map='auto'
    )
    model.train()
    ...
    from omnigen2.models.sdpa_npu_redirect import apply_sdpa_npu_redirect
    apply_sdpa_npu_redirect(verbose=True)
    ...
    for epoch in range(first_epoch, args.train.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                bin_indices = torch.bucketize(loss_dict["t"].cuda().float(), loss_bins, right=True) - 1
