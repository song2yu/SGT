...
def generate_images(
    ...
    for i in range(num_images):
        ...
        results = pipeline(
            ...
            num_images_per_prompt=1,  # Force one image per call so the seeds stay distinct.
            generator=generator,
            output_type="pil",
        )
...
def main():
    ...
    # Synchronize after model loading so every GPU is ready before starting.
    if args.use_distributed:
        dist.barrier()
        if rank == 0:
